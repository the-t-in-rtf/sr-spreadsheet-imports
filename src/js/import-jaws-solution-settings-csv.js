...
var existingSolutionKeys = Object.keys(fluid.get(existingSolutionData,["com.freedomscientific.jaws",  "settingsHandlers", "configuration1", "supportedSettings"]));

fs.writeFileSync("/tmp/existing-solution-keys.txt", JSON.stringify(existingSolutionKeys, null, 2));

var csvPath = "/Users/duhrer/Downloads/JAWS Product & Metadata File - Metadata.csv";
...
var excludedGroup = function (candidate) {
    if (!candidate || !candidate.id) {
        return true;
    }

    var toExclude = ["focus", "PMDisplay20", "PMDisplay40", "WindowClasses", "MSAAClasses", "RichEdit and Edit Control Options", "SynthAliases"];
    var segments = candidate.id.split(".");
    if (segments.length > 1) {
        return toExclude.indexOf(segments[0]) !== -1;
    }
    return false;
};
...
var csvStream = csv({ delimiter: ","})
    .on("data", function (data) {
        if (rowNumber > 0) {
            ...
            if (rowData.onboard && rowData.onboard.indexOf("Done") !== -1) {
            ...
            else if (rowData.onboard && rowData.onboard === "No") {
                console.log("Skipping declined record '" + rowData.title + "'.");
            }
            else if (excludedGroup(rowData)) {
                console.log("Skipping record '" + rowData.id + "' because it's in an excluded group.");
            }
            else if (missingData(rowData)) {
                return;
            }
            else if (existingSolutionKeys.indexOf(rowData["machine name"]) !== -1) {
                console.log("Skipping record '" + rowData.title + "' that has already been onboarded.");
            }
            else {
                var key = rowData.id;
                ...
                if (rowData.type === "Binary") {
                    ...
                    candidateSolutionSetting["enum"] = rowData["machine value space"]; // We know this will be broken and cleaned up manually.
                    candidateSolutionSetting.enumLabels = rowData["user value space"];
                    if (rowData["default"]) {
                        candidateSolutionSetting["default"] = rowData["default"];
                    }
                }
    ...
    .on("end", function () {
        ...
        var outputPath = "/tmp/jaws-candidate-solutions.json";
